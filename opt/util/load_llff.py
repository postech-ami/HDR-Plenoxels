...
def render_path_spiral_before(c2w, up, rads, focal, zrate, rots, N):
    render_poses = []
...
def render_path_spiral(c2w, up, rads, focal, zrate, rots, N, rads_ratio=0.6, zrate_ratio=0.8):
    render_poses = []
    
    print("rads ratio:", rads_ratio)
    print("zrate_Ratio:", zrate_ratio)
    # zrate small -> more far to center
    ...
    for theta in np.linspace(0.0, 2.0 * np.pi * rots, N + 1)[:-1]:
        c = np.dot(
            ...
            # rads
            # np.array([np.cos(theta), -np.sin(theta), -np.sin(theta * zrate), 1.0])
            np.array([np.cos(theta), -np.sin(theta), -np.sin(np.pi * zrate_ratio * zrate) - 0.5, 1.0])
            * rads,
        )
        z = normalize(c - np.dot(c2w[:3, :4], np.array([0, 0, -focal, 1.0])))
        # render_poses.append(np.concatenate([viewmatrix(z, up, c), hwf], 1))
...
def load_llff_data(
    ...
    if spherify:
        ...
        print("pose shape:", poses.shape)  # (50, 3, 4)
        ## Get spiral
        # Get average pose
